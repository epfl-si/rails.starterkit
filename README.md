...
- docker-compose version 2 (there is a [switch](https://github.com/docker/compose/issues/1123#issuecomment-1129313318) to flip in Docker Desktop for Mac for this)
...
2. Run <pre>./bin/dev</pre>

The latter starts up all the development things, with hot-rebuild everywhere: server-side with Puma and turbo-rails, as well as client-side with esbuild.
...
Should you wish to purge everything including the Keycloak state in MariaDB, say
...
As per the book on page 11–...

1. Create realm `rails`
2. Create user `demo` in realm (Users → ⊕)
3. Set password for user `demo` (Users → `demo` → Credentials)
   - Be sure to deselect `Temporary`
